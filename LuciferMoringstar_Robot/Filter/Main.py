...
async def filter(client, message):
    ...
    if AUTH_CHANNEL:
        ...
        try:
            ...
            await client.send_message(
                chat_id=message.from_user.id,
                text="**✌🏻 നമസ്കാരം ✌🏻**/n/n**സിനിമകൾ കിട്ടണമെങ്കിൽ༆**/n⍟ "𝙹𝙾𝙸𝙽✨" Click ചെയ്ത്  🎞 𝙼𝙻𝙰 𝙺𝙸𝙽𝙶𝙳𝙾𝙼 🎞 ചാനലിൽ ജോയിൻ ചെയ്യുക/n⍟ 𝚁𝙴𝙵𝚁𝙴𝚂𝙷♻️ Click ചെയ്യുക/n⍟ Start കൊടുക്കുക.",
...
async def group(client, message):
    ...
    if 2 < len(message.text) < 50:    
        btn = []
        search = message.text
        mo_tech_yt = f"**🗂️ Title:** {search}\n**⭐ Rating:** {random.choice(RATING)}\n**🎭 Genre:** {random.choice(GENRES)}\n** How to Download**/n**Pin ചെയ്ത Video കാണൂ "
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if (clicked == typed):
        ...
        if query.data.startswith("next"):
            ...
            buttons = [
                [
                    InlineKeyboardButton('JOIN BACKUP CHANNEL', url=f'{TUTORIAL}')
                ]
                ]
